...
RMMVData.prototype.fetchChoices = function(pageList, index, context) {
	...
	*/
	...
	// the next command is the first choice with the same indentation
	//var firstChoice = pageList[index+1];
	//var thisText 	= firstChoice.parameters[1];
	//firstChoice.parameters[1] = choices[0].translation;
	var processed = 0;
	...
	for (var i=index+1; i<pageList.length; i++) {
		...
		// this iteration do not process first choice
		// because first choice has the same indentation with the parent
		// nested choice will be in chaos if processed here
		// if (currentPage.parameters[0] == 0) continue; 
...
RMMVGames.determineRootDir = async function(gameDir) {
	if (await common.isDirectory(gameDir) == false) return console.warn(gameDir, "is not a valid directory");
...
RMMVGames.prototype.parse = async function($MV_PATH, options) {
	...
	*/
	...
	for (var $key in files) {
		...
		console.warn("相对路径是：", relativePath);
...
RMMVGames.prototype.exportToFolder = async function(targetFolder, options) {
	...
	for (var path in this.parsed) {
		...
		ui.log("创建目录（如果不存在）：", nwPath.dirname(newFile));
		await common.mkDir(nwPath.dirname(newFile));
		ui.log("正在写入文件：", newFile);
...
RMMVDecrypter.prototype.loadSystem = async function() {
	try {
		...
		alert(t("加载 system.json 时出错")+e.toString());
	}
}
...
$(document).ready(function() {
		...
		try {
			if (await await common.isDirectory(targetPath)) {
				...
				if (Boolean(thisAddon.config.useLegacyParser) == false) {
					...
					var rmmvGames = new addonLoader.addons.rmmv.RMMVGames(entryPoint);
	...
	engines.addHandler(["rmmv", "rmmz"], 'injectHandler', async function(targetDir, sourceMaterial, options) {
		...
		await ui.log(`--从中复制材料 ${sourceDir} 到 ${targetDir}`);

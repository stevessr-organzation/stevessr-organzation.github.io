...
json.scalarFallback = str => {
  throw new SyntaxError(`未解析纯标量 ${JSON.stringify(str)}`);
};
...
function findTagObject(value, tagName, tags) {
  if (tagName) {
    ...
    if (!tagObj) throw new Error(`标签 ${tagName} 找不到`);
    return tagObj;
...
function getSchemaTags(schemas, knownTags, customTags, schemaId) {
  ...
  if (!tags) {
    ...
    throw new Error(`未知模式 "${schemaId}"; 使用其中一个 ${keys}`);
  }
  ...
  for (let i = 0; i < tags.length; ++i) {
    ...
    if (typeof tag === 'string') {
      ...
      if (!tagObj) {
        ...
        throw new Error(`未知自定义标记 "${tag}"; 使用其中一个 ${keys}`);
      }

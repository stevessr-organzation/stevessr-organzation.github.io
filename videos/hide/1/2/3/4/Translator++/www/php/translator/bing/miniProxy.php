...
define("PROXY_PREFIX", "http" . "://" . $prefixHost . $prefixPort . $_SERVER["SCRIPT_NAME"] . "?");
...
$header_blacklist_pattern = "/^Content-Length|^Transfer-Encoding|^X-Frame-Options|^X-XSS-Protection|^Content-Encoding.*gzip/i";
...
} else if (strpos($url, 'https://translate.google.com/translate_a/single?' ) === 0) {
	// donovan addition
	file_put_contents("cache.txt", $responseBody);

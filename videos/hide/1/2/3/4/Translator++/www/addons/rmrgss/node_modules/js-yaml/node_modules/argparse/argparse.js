...
function range(from, to, step=1) {
    ...
    if (typeof from !== 'number' || typeof to !== 'number' || typeof step !== 'number') {
        throw new TypeError('参数不能解释为整数')
    }
    if (step === 0) throw new TypeError('range() arg 3 不能为零')
...
function _array_remove(array, item) {
    ...
    if (idx === -1) throw new TypeError(sub('%r不在列表中', item))
    array.splice(idx, 1)
}
...
function _choices_to_array(choices) {
    if (choices === undefined) {
        ...
        throw new Error(sub('无效的选择值：%r', choices))
    }
}
...
function _alias(object, from, to) {
    try {
        ...
        Object.defineProperty(object, from, {
            value: util.deprecate(object[to], sub('%s.%s() 已重命名为 %s.%s()',
                name, from, name, to)),
            enumerable: false
        })
    } catch {}
...
function _parse_opts(args, descriptor) {
    ...
    if (renames.length) {
        let name = get_name()
        deprecate('camelcase_' + name, sub('%s(): 以下选项被重命名：%s',
    ...
    for (let [ key, def ] of Object.entries(descriptor)) {
        if (key[0] === '*') {
            ...
            throw new TypeError(sub('%s()为参数%r获取了多个值', get_name(), key))
    ...
    if (Object.keys(kwargs).length) {
        throw new TypeError(sub('%s() 得到了一个意外的关键字参数 %r',
    ...
    if (args.length) {
        ...
        throw new TypeError(sub('%s() 接受%s位置参数%s，但给定%s %s',
            get_name(),
    ...
    if (missing_positionals.length) {
        ...
        throw new TypeError(sub('%s() 缺少%i所需的位置参数%s: %s',
...
const HelpFormatter = _camelcase_alias(_callable(class HelpFormatter {
    ...
    _dedent() {
        this._current_indent -= this._indent_increment
        assert(this._current_indent >= 0, '缩进量降至0以下。')
        this._level -= 1
    }
...
const ArgumentError = _callable(class ArgumentError extends Error {
    ...
    str() {
        ...
        if (this._argument_name === undefined) {
            ...
            format = '论点 %(argument_name)s: %(message)s'
        }
...
const Action = _camelcase_alias(_callable(class Action extends _AttributeHolder(Function) {
    ...
    constructor() {
        ...
        super('返回arguments.callee.call.apply(arguments.callee，arguments)')
...
const _StoreAction = _callable(class _StoreAction extends Action {
    ...
    constructor() {
        ...
        if (nargs === 0) {
            throw new TypeError('存储行为的NARG必须是 != 0; 如果你' +
                        '没有要存储的内容，例如存储' +
                        'true或store const可能更合适')
        }
        if (const_value !== undefined && nargs !== OPTIONAL) {
            throw new TypeError(sub('NARG必须为%r才能提供常数', OPTIONAL))
        }
        super({
            option_strings,
            dest,
...
const _AppendAction = _callable(class _AppendAction extends Action {
    ...
    constructor() {
        ...
        if (nargs === 0) {
            throw new TypeError('附加操作的NARG必须是!= 0;如果arg' +
                        '字符串没有提供要追加的值，' +
                        'append const操作可能更合适')
        }
        if (const_value !== undefined && nargs !== OPTIONAL) {
...
const _SubParsersAction = _camelcase_alias(_callable(class _SubParsersAction extends Action {
    ...
    call(parser, namespace, values/*, option_string = undefined*/) {
        ...
        if (hasattr(this._name_parser_map, parser_name)) {
            ...
            let msg = sub('未知分析器 %(parser_name)r (choices: %(choices)s)', args)
...
const FileType = _callable(class FileType extends Function {
    ...
    constructor() {
        ...
        // when this class is called as a function, redirect it to .call() method of itself
        super('返回 arguments.callee.call.apply(arguments.callee, arguments)')
    ...
    call(string) {
        ...
        if (string === '-') {
            if (this._flags.includes('r')) {
                ...
                let msg = sub('参数"-"，模式为%r', this._flags)
        ...
        try {
            ...
            let message = "打不开 '%(filename)s': %(error)s"
        ...
        if (this._flags.includes('r')) {
            ...
            let msg = sub('参数“%s”的模式为%r', string, this._flags)
            throw new TypeError(msg)
        }
    }
    ...
    constructor() {
        ...
        ;[ 'storeConst', 'storeTrue', 'storeFalse', 'appendConst' ].forEach(old_name => {
            ...
            this.register('action', old_name, util.deprecate(this._registry_get('action', new_name),
                sub('{action: "%s"} 更名为 {action: "%s"}', old_name, new_name)))
        })
        // end
    ...
    add_argument() {
        ...
        if (args.length === 1 && Array.isArray(args[0])) {
            ...
            deprecate('argument-array',
                sub('使用 add_argument(%(args)s, {...}) 代替 add_argument([ %(args)s ], { ... })', {
        ...
        if (!args.length || args.length === 1 && !chars.includes(args[0][0])) {
            if (args.length && 'dest' in kwargs) {
                throw new TypeError('dest为位置参数提供了两次')
            }
        ...
        if (typeof action_class !== 'function') {
            throw new TypeError(sub('未知操作“%s”', action_class))
        }
        ...
        if (typeof type_func !== 'function') {
            throw new TypeError(sub('%r不可调用', type_func))
        }

        if (type_func === FileType) {
            throw new TypeError(sub('%r是一个文件类型类对象，是它的实例' +
                                    '必须通过', type_func))
        }
        ...
        if ('_get_formatter' in this) {
            try {
                ...
                if (err instanceof TypeError && err.message !== 'invalid nargs value') {
                    throw new TypeError('metavar元组的长度与NARG不匹配')
                } else {
                    throw err
                }
            }
    ...
    _get_optional_kwargs() {
        ...
        for (option_string of args) {
            ...
            if (!this.prefix_chars.includes(option_string[0])) {
                ...
                let msg = '无效的选项字符串 %(option)r: ' +
                          '必须从一个字符开始 %(prefix_chars)r'
...
const ArgumentParser = _camelcase_alias(_callable(class ArgumentParser extends _AttributeHolder(_ActionsContainer) {
    ...
    constructor() {
        ...
        if (debug !== undefined) {
            deprecate('debug',
                'ArgumentParser的“debug”参数已被弃用。请' +
                '重写参数分析器。取而代之的是退出功能。'
            )
        }
        ...
        if (version !== undefined) {
            deprecate('version',
                'ArgumentParser的“version”参数已被弃用。请使用' +
        ...
        this.register('type', 'int', function (x) {
            ...
            if (!Number.isInteger(result)) {
                throw new TypeError(sub('无法将字符串转换为整数：%r', x))
            }
            return result
        })
        this.register('type', 'float', function (x) {
            let result = Number(x)
            if (isNaN(result)) {
                throw new TypeError(sub('无法将字符串转换为浮点：%r', x))
            }
            return result
        })
        ...
        this.register('type', 'string',
            util.deprecate(String, '使用 {type:"str"} 或 {type:String} 代替 {type:"string"}'))
        // end
    ...
    _parse_optional(arg_string) {
        ...
        if (option_tuples.length > 1) {
            ...
            let msg = '选项不明确：%(option)s 可能匹配 %(matches)s'
    ...
    _get_option_tuples(option_string) {
        ...
        // shouldn't ever get here
            ...
            this.error(sub('意外的选项字符串：%s', option_string))
        }
    ...
    parse_known_intermixed_args(args = undefined, namespace = undefined) {
        ...
        if (a.length) {
            throw new TypeError(sub('parse_intermixed_args: 位置参数' +
        ...
        for (let group of this._mutually_exclusive_groups) {
            for (let action of group._group_actions) {
                if (positionals.includes(action)) {
                    throw new TypeError('parse_intermixed_args: 定位于' +
            ...
            try {
                ...
                for (let action of positionals) {
                    ...
                    if (Array.isArray(attr) && attr.length === 0) {
                        ...
                        console.warn(sub('不要期望%s出现在%s中', action.dest, namespace))
    ...
    _check_value(action, value) {
        ...
        if (action.choices !== undefined && !_choices_to_array(action.choices).includes(value)) {
            ...
            let msg = '无效选择：%(value)r (choose from %(choices)s)'
            throw new ArgumentError(action, sub(msg, args))
        }
    }
    ...
    error(message) {
        ...
        this.exit(2, sub('%(prog)s: 错误：%(message)s\n', args))
    }
}))
    ...
    get() {
        ...
        Object.entries({ ONE_OR_MORE, OPTIONAL, PARSER, REMAINDER, SUPPRESS, ZERO_OR_MORE }).forEach(([ n, v ]) => {
            Object.defineProperty(result, n, {
                get() {
                    deprecate(n, sub('使用 argparse.%s 代替 argparse.Const.%s', n, n))
                    return v
                }
            })
        })
        Object.entries({ _UNRECOGNIZED_ARGS_ATTR }).forEach(([ n, v ]) => {
            Object.defineProperty(result, n, {
                get() {
                    deprecate(n, sub('argparse.Const.%s 是一个内部符号，将不再可用', n))
                    return v
                }
            })
        })

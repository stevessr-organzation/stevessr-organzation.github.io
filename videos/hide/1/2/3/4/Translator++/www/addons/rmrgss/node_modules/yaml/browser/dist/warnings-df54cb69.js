...
var binary = {
  ...
  resolve: function resolve(doc, node) {
    ...
    if (typeof Buffer === 'function') {
      ...
      var msg = '此环境不支持读取二进制标记；需要缓冲区或atob';
  ...
  stringify: function stringify(_ref, ctx, onComment, onChompKeep) {
    ...
    if (typeof Buffer === 'function') {
      ...
      throw new Error('此环境不支持写入二进制标记；需要缓冲区或btoa');
    }
    ...
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      ...
      if (Array.isArray(it)) {
        if (it.length === 2) {
          key = it[0];
          value = it[1];
        } else throw new TypeError("预期的 [key, value] 数组: ".concat(it));
    ...
    value: function toJSON$1(_, ctx) {
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          if (map.has(key)) throw new Error('有序映射不得包含重复的密钥');
          map.set(key, value);
        }
      } catch (err) {
        _iterator.e(err);
...
var YAMLSet = /*#__PURE__*/function (_YAMLMap) {
  ...
  _createClass(YAMLSet, [{
    ...
    value: function set(key, value) {
      if (typeof value !== 'boolean') throw new Error("YAML集合中 set(key, value) 的预期布尔值，而不是".concat(_typeof(value)));
    ...
    value: function toString(ctx, onComment, onChompKeep) {
      ...
      if (this.hasAllNullValues()) return _get(_getPrototypeOf(YAMLSet.prototype), "toString", this).call(this, ctx, onComment, onChompKeep);else throw new Error('集合项必须全部为空值');
    }
  }]);
...
function parseSet(doc, cst) {
  ...
  if (!map.hasAllNullValues()) throw new YAMLSemanticError(cst, '集合项必须全部为空值');
...
function warnFileDeprecation(filename) {
  if (shouldWarn(true)) {
    ...
    warn("终点 'yaml/".concat(path, "' 将在以后的版本中删除。"), 'DeprecationWarning');
  }
}
var warned = {};
function warnOptionDeprecation(name, alternative) {
  if (!warned[name] && shouldWarn(true)) {
    warned[name] = true;
    var msg = "选择 '".concat(name, "' 将在以后的版本中删除");

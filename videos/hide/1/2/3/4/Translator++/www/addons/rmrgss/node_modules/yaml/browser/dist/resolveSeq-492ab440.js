  ...
  _createClass(Collection, [{
    ...
    value: function addIn(path, value) {
      if (isEmptyPath(path)) this.add(value);else {
        ...
        if (node instanceof Collection) node.addIn(rest, value);else if (node === undefined && this.schema) this.set(key, collectionFromPath(this.schema, rest, value));else throw new Error("预计YAML收集时间为".concat(key, "。剩余路径：").concat(rest));
      }
    }
  }, {
    key: "deleteIn",
    value: function deleteIn(_ref) {
      ...
      var node = this.get(key, true);
      if (node instanceof Collection) return node.deleteIn(rest);else throw new Error("预计YAML收集时间为".concat(key, "。剩余路径：").concat(rest));
    }
  }, {
    key: "getIn",
    ...
    value: function setIn(_ref7, value) {
      ...
      if (rest.length === 0) {
        ...
        var node = this.get(key, true);
        if (node instanceof Collection) node.setIn(rest, value);else if (node === undefined && this.schema) this.set(key, collectionFromPath(this.schema, rest, value));else throw new Error("预计YAML收集时间为".concat(key, "。剩余路径：").concat(rest));
      }
...
var YAMLSeq = /*#__PURE__*/function (_Collection) {
  ...
  _createClass(YAMLSeq, [{
    ...
    value: function set(key, value) {
      var idx = asItemIndex(key);
      if (typeof idx !== 'number') throw new Error("应为有效索引，而不是".concat(key, "."));
  ...
  _createClass(Pair, [{
    ...
    set: function set(cb) {
      ...
      if (this.key instanceof Node) this.key.commentBefore = cb;else {
        var msg = 'Pair.commentBefore 是Pair.key.commentBefore 的别名。要设置它，键必须是一个Node。';
        throw new Error(msg);
      }
    }
  }, {
    ...
    value: function toString(ctx, onComment, onChompKeep) {
      ...
      if (simpleKeys) {
        if (keyComment) {
          throw new Error('对于简单键，键节点不能有注释');
        }

        if (key instanceof Collection) {
          var msg = '对于简单键，集合不能用作键值';
          throw new Error(msg);
        }
      }
      ...
      if (!explicitKey && str.length > 1024) {
        if (simpleKeys) throw new Error('对于简单键，单行标量的跨度不得超过1024个字符');
        explicitKey = true;
      }
...
var Alias = /*#__PURE__*/function (_Node) {
  ...
  _createClass(Alias, [{
    ...
    value: function stringify(_ref, _ref2) {
      ...
      var msg = doc.anchors.getName(source) ? '别名节点必须位于源节点之后' : '找不到别名节点的源节点';
...
var YAMLMap = /*#__PURE__*/function (_Collection) {
  ...
  _createClass(YAMLMap, [{
    ...
    value: function add(pair, overwrite) {
      ...
      if (prev) {
        if (overwrite) prev.value = pair.value;else throw new Error("密钥".concat(pair.key, "已经设定"));
      } else if (sortEntries) {
    ...
    value: function toString(ctx, onComment, onChompKeep) {
      ...
      try {
        for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
          ...
          if (!(item instanceof Pair)) throw new Error("地图项目必须是成对的；建立".concat(JSON.stringify(item), " 代替"));
        }
      } catch (err) {
        _iterator3.e(err);
      } finally {
    ...
    value: function addToJSMap(ctx, map) {
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          if (!(source instanceof YAMLMap)) throw new Error('合并源必须是地图');
...
function stringifyString(item, ctx, onComment, onChompKeep) {
  ...
  if (res === null) {
    ...
    if (res === null) throw new Error("不支持的默认字符串类型".concat(defaultType));
  }
...
function checkFlowCollectionEnd(errors, cst) {
  ...
  switch (cst.type) {
    ...
    default:
      errors.push(new YAMLSemanticError(cst, '不是流量收集！？'));
      return;
  }
  ...
  if (lastItem && lastItem.char !== char) {
    var msg = "预期".concat(name, "以结束").concat(char);
    var err;
...
function getLongKeyError(source, key) {
  ...
  return new YAMLSemanticError(source, "这 \"".concat(k, "\" 密钥太长了"));
}
...
function resolveTagHandle(doc, node) {
  ...
  if (!prefix) {
    ...
    if (!prefix) throw new YAMLSemanticError(node, "这".concat(handle, " 标记句柄是非默认的，未声明。"));
  }

  if (!suffix) throw new YAMLSemanticError(node, "这".concat(handle, " 标签没有后缀。"));

  if (handle === '!' && (doc.version || doc.options.version) === '1.0') {
    if (suffix[0] === '^') {
      doc.warnings.push(new YAMLWarning(node, 'YAML 1.0^不支持标签扩展'));
      return suffix;
    }
...
function resolveTagName(doc, node) {
  ...
  if (tag) {
    ...
    if (verbatim) {
      ...
      var msg = "逐字标记没有解析，所以".concat(verbatim, "无效。");
...
function resolveTag(doc, node, tagName) {
  ...
  try {
    ...
    if (!fallback) throw new Error("标签".concat(tagName, "不可用"));
    var msg = "标签".concat(tagName, "不可用，正在退回到").concat(fallback);
...
function resolveNodeValue(doc, node) {
  ...
  if (node.type !== Type.PLAIN) {
    var _msg3 = "未能解决".concat(node.type, "节点在这里");
...
function resolveMap(doc, cst) {
  if (cst.type !== Type.MAP && cst.type !== Type.FLOW_MAP) {
    var msg = "一个".concat(cst.type, "无法将节点解析为映射");
    ...
    if (doc.schema.merge && iKey && iKey.value === MERGE_KEY) {
      ...
      sources.some(function (node) {
        if (node instanceof Alias) {
          ...
          return error = '合并节点别名只能指向贴图';
        }

        return error = '合并节点只能将别名节点作为值';
      });
      ...
      for (var j = i + 1; j < items.length; ++j) {
        ...
        if (iKey === jKey || iKey && jKey && Object.prototype.hasOwnProperty.call(iKey, 'value') && iKey.value === jKey.value) {
          var _msg = "地图键必须是唯一的； \"".concat(iKey, "\" is repeated");
  ...
  if (hasCollectionKey && !doc.options.mapAsMap) {
    var warn = '由于JS对象限制，具有集合值的键将被字符串化为YAML。使用mapAsMap:true可以避免这种情况。';
...
function resolveFlowMapItems(doc, cst) {
  ...
  for (var i = 0; i < cst.items.length; ++i) {
    ...
    if (typeof item.char === 'string') {
      ...
      if (next === ',') doc.errors.push(new YAMLSemanticError(item, '流程图中缺少分隔符'));
      key = resolveNode(doc, item);
    } else {
      if (next !== ',') doc.errors.push(new YAMLSemanticError(item, '指示器：流程图条目中缺少'));
...
function resolveSeq(doc, cst) {
  if (cst.type !== Type.SEQ && cst.type !== Type.FLOW_SEQ) {
    var msg = "A ".concat(cst.type, "无法将节点解析为序列");
    doc.errors.push(new YAMLSyntaxError(cst, msg));
    return null;
  }
...
function resolveBlockSeqItems(doc, cst) {
  ...
  for (var i = 0; i < cst.items.length; ++i) {
    ...
    switch (item.type) {
      ...
      default:
        if (item.error) doc.errors.push(item.error);
        doc.errors.push(new YAMLSyntaxError(item, "顺序中出现意外的".concat(item.type, "节点")));
    }
  }
  ...
  for (var i = 0; i < cst.items.length; ++i) {
    ...
    if (typeof item.char === 'string') {
      ...
      if (char === next) {
        ...
        var _msg2 = "流序列包含意外的".concat(char);
      ...
      if (next) {
        var _msg3 = "需要".concat(next, "按顺序排列");

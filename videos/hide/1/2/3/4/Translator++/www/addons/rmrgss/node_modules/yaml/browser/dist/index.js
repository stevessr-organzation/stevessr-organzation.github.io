...
function getTagObject(tags, item) {
  ...
  if (!tagObj) {
    ...
    throw new Error("标签未解析为".concat(name, " value"));
  }
  ...
  _createClass(Anchors, [{
    ...
    value: function createMergePair() {
      ...
      merge.value.items = sources.map(function (s) {
        ...
        throw new Error('合并源必须是映射节点或其别名');
      });
      return merge;
    }
  }, {
    ...
    value: function setAnchor(node, name) {
      if (node != null && !Anchors.validAnchorNode(node)) {
        throw new Error('只能为标量、Seq和Map节点设置锚定');
      }

      if (name && /[\x00-\x19\s,[\]{}]/.test(name)) {
        throw new Error('锚点名称不能包含空格或控制字符');
      }
...
function resolveTagDirective(_ref, directive) {
  ...
  if (tagPrefixes.some(function (p) {
    ...
    var _msg = '同一文档中每个句柄最多只能给出一次%TAG指令。';
...
function resolveYamlDirective(doc, directive) {
  ...
  if (!documentOptions[version]) {
    ...
    var _msg2 = "文档将被解析为YAML.concat（v0），而不是YAML").concat(version);
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      ...
      switch (name) {
        ...
        default:
          if (name) {
            var _msg3 = "YAML只支持%TAG和%YAML指令，不支持%".concat(name);
...
function assertCollection(contents) {
  ...
  throw new Error('应将YAML集合作为文档内容');
}
...
var Document$1 = /*#__PURE__*/function () {
  ...
  _createClass(Document, [{
    ...
    value: function setTagPrefix(handle, prefix) {
      if (handle[0] !== '!' || handle[handle.length - 1] !== '!') throw new Error('手柄必须以开始和结束！');
    ...
    value: function toString() {
      if (this.errors.length > 0) throw new Error('有错误的文档无法字符串化');
      var indentSize = this.options.indent;

      if (!Number.isInteger(indentSize) || indentSize <= 0) {
        var s = JSON.stringify(indentSize);
        throw new Error("“缩进”选项必须是正整数，而不是".concat(s));
      }
...
function parseDocument(src, options) {
  ...
  if (cst.length > 1) {
    var errMsg = '源包含多个文档；请使用YAML.parseAllDocuments（）';

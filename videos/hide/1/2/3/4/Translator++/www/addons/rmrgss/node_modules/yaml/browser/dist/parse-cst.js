...
var CollectionItem = /*#__PURE__*/function (_Node) {
  ...
  _createClass(CollectionItem, [{
    ...
    value: function parse(context, start) {
      ...
      if (!atLineStart && this.type === Type.SEQ_ITEM) this.error = new YAMLSemanticError(this, '序列项不能在同一行上有前面的内容');
    ...
    value: function parseDirectives(start) {
      ...
      while (!Node.atDocumentBoundary(src, offset, Char.DIRECTIVES_END)) {
        ...
        switch (src[offset]) {
          ...
          default:
            if (hasDirectives) {
              this.error = new YAMLSemanticError(this, '缺少结束指示行');
      ...
      if (src[offset]) {
        ...
        if (src[offset]) {
          ...
          switch (src[offset]) {
            ...
            default:
              this.error = new YAMLSyntaxError(this, '文档结束标记行不能有非注释后缀');
          }
        }
      }
        ...
        if (!this.blockIndent) {
          ...
          if (src[end] !== '\n') {
            ...
            if (lineIndent < minBlockIndent) {
              var msg = '前导空行缩进较多的块标量必须使用显式缩进指示符';
          ...
          if (!this.error) {
            var _src = explicit ? '显式缩进指示器' : '第一行';

            var _msg = "块标量的缩进程度不得小于其".concat(_src);
  ...
  _createClass(QuoteDouble, [{
    ...
    function get() {
      ...
      var _this$context = this.context,
          indent = _this$context.indent,
          src = _this$context.src;
      if (src[end - 1] !== '"') errors.push(new YAMLSyntaxError(this, '缺少结束”引用')); // Using String#replace is too painful with escaped newlines preceded by
      ...
      for (var i = start + 1; i < end - 1; ++i) {
        ...
        if (ch === '\n') {
          if (Node.atDocumentBoundary(src, i + 1)) errors.push(new YAMLSemanticError(this, '字符串值中不允许使用文档边界指示符'));
          ...
          str += fold;
          i = offset;
          if (error) errors.push(new YAMLSemanticError(this, '多行双引号字符串需要充分缩进'));
        } else if (ch === '\\') {
          i += 1;
          ...
          switch (src[i]) {
            ...
            default:
              errors.push(new YAMLSyntaxError(this, "无效的转义序列".concat(src.substr(i - 1, 2))));
              str += '\\' + src[i];
          }
        } else if (ch === ' ' || ch === '\t') {
    ...
    value: function parseCharCode(offset, length, errors) {
      ...
      if (isNaN(code)) {
        errors.push(new YAMLSyntaxError(this, "无效的转义序列".concat(src.substr(offset - 2, length + 2))));
  ...
  _createClass(QuoteSingle, [{
    ...
    function get() {
      ...
      var _this$context = this.context,
          indent = _this$context.indent,
          src = _this$context.src;
      if (src[end - 1] !== "'") errors.push(new YAMLSyntaxError(this, "缺少结束”引用"));
      var str = '';
      ...
      for (var i = start + 1; i < end - 1; ++i) {
        ...
        if (ch === '\n') {
          ...
          str += fold;
          i = offset;
          if (error) errors.push(new YAMLSemanticError(this, '多行单引号字符串需要充分缩进'));
        } else if (ch === "'") {
          str += ch;
          i += 1;
          if (src[i] !== "'") errors.push(new YAMLSyntaxError(this, '未替换的单引号？这不应该发生。'));
        } else if (ch === ' ' || ch === '\t') {
          // trim trailing whitespace
          var wsStart = i;
          var next = src[i + 1];
  ...
  function ParseContext() {
    ...
    _defineProperty(this, "parseNode", function (overlay, start) {
      ...
      if (offset <= start) {
        ...
        node.error = new Error("节点#不使用任何字符");
      ...
      if (context.nodeStartsCollection(node)) {
        if (!node.error && !context.atLineStart && context.parent.type === Type.DOCUMENT) {
          node.error = new YAMLSyntaxError(node, '块集合在此不能有前面的内容（例如指令结束指示器）');
        }

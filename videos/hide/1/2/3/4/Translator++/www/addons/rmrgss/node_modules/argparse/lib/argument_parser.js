...
function ArgumentParser(options) {
  ...
  this._positionals = this.addArgumentGroup({ title: '位置参数' });
  this._optionals = this.addArgumentGroup({ title: '可选参数' });
        ...
        actionConflicts[actionHash(action)].forEach(function (actionConflict) {
          if (seenNonDefaultActions.indexOf(actionConflict) >= 0) {
            throw argumentErrorHelper(
              action,
              format('参数“%s”不允许使用。', actionConflict.getName())
            );
          }
        });
      }
      ...
      if (explicitArg) {
        ...
        if (argCount === 0 && chars.indexOf(optionString[1]) < 0) {
          ...
          if (Object.keys(optionalsMap).indexOf(optionString) >= 0) {
            ...
            throw argumentErrorHelper(action, sprintf('已忽略显式参数%r', explicitArg));
          }
        } else if (argCount === 1) {
  ...
  self._actions.forEach(function (action) {
    if (action.required) {
      if (seenActions.indexOf(action) < 0) {
        self.error(format('参数“%s”是必需的', action.getName()));
      }
    }
  });
...
ArgumentParser.prototype._getOptionTuples = function (optionString) {
  ...
  // shouldn't ever get here
    ...
    throw new Error(format('意外的选项字符串：%s。', optionString));
  }
...
ArgumentParser.prototype._getValue = function (action, argString) {
  ...
  if (typeof typeFunction !== 'function') {
    var message = format('%s是不可调用的', typeFunction);
  ...
  try {
    ...
    var msg = format('无效的%s值：%s', name, argString);
...
ArgumentParser.prototype._checkValue = function (action, value) {
  ...
  if (choices) {
    ...
    var message = format('无效选择：%s（从[%s]中选择）', value, choices);
    throw argumentErrorHelper(action, message);
  }
};
...
ArgumentParser.prototype.error = function (err) {
  ...
  var msg = format('%s:错误：%s', this.prog, message) + c.EOL;

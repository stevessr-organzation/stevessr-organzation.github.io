...
function splitByNewline(str) {
    return str.split('\r\n').reduce(function (p, s) {
        p.push.apply(p, s.split(/\r|\n/));
        return p;
    }, []);
}
function stepLocation(s, step) {
    if (step === void 0) { step = 1; }
    var jumped = s.src.substr(s.ptr, step);
    var lines = splitByNewline(jumped);
...
function nextIs(s, str) {
    ...
    for (var _i = 0, _str_1 = _str; _i < _str_1.length; _i++) {
        ...
        var actual = s.src.substr(s.ptr, _s.length);
        if (actual === _s)
            return true;
    }
    return false;
...
function stepIf(s, str) {
    var _str = strParamToArray(str);
    for (var _i = 0, _str_2 = _str; _i < _str_2.length; _i++) {
        var _s = _str_2[_i];
        if (nextIs(s, _s)) {
...
function curChar(s) {
    if (s.ptr > s.src.length)
        common_1.e(s, 'Current char Read over EOF');
    return s.src[s.ptr];
}
...
function nextChar(s) {
    if (s.ptr + 1 > s.src.length)
        common_1.e(s, 'Next char Read over EOF');
...
function isSpace(str) {
    ...
    // No, they don't... V8 only?
        ...
        return str.split('').every(function (k) { return tokens_1.pZs.includes(k); });
}
...
function readNonQuoteString(s, until) {
    var ret = '';
    var _until = strParamToArray(until);
...
function readEscapeSequence(c, s) {
    if (!stepIf(s, '\\'))
        common_1.e(s, 'Expected Escape sequence:' + curChar(s));
    var ch = curChar(s);
    switch (ch) {
        case 'u':
            if (!c.unicodeEscape)
                common_1.e(s, '\\u support disabled');
            // JS Unicode mode
            // \uxxxx
            // \u{??????}
            var digit1 = stepChar(s);
            var cpStr = '';
            if (digit1 === '{') {
                do {
                    cpStr += stepChar(s);
            ...
            if (!cpStr.match(/^[0-9a-f]+$/i)) {
                common_1.e(s, 'Unexpected Unicode codepoint');
            }
            var cpNum = parseInt(cpStr, 16);
...
function readQuotedString(c, s) {
    var q = curChar(s);
    if (q !== '"' && q !== "'") {
        common_1.e(s, 'Unexpected quote char, actual:' + q);
    }
    stepIf(s, q);
    var r = '';
    outerLoop: while (1) {
        switch (curChar(s)) {
            ...
            case '\r':
            case '\n':
                common_1.e(s, 'newline is not allowed');
            default:
                r += curChar(s);
                stepChar(s);
                break;
